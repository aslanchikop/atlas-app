...
def get_css():
    """Generate beautiful CSS based on current theme"""
    theme = st.session_state.get('theme', 'dark')
...
/* ═══════════════════════════════════════════════════════════════════════════
   MAIN BACKGROUND - Cosmic gradient with stars
   ═══════════════════════════════════════════════════════════════════════════ */
...
.stApp {{
    background: 
        ...
        linear-gradient(180deg, #0a0a0f 0%, #0f0f1a 50%, #0a0a0f 100%) !important;
    min-height: 100vh;
}}
...
:root {{
    ...
    --gradient-1: linear-gradient(135deg, #667eea 0%, #764ba2 100%);
    --gradient-2: linear-gradient(135deg, #0891b2 0%, #059669 100%);
...
/* ═══════════════════════════════════════════════════════════════════════════
   MAIN BACKGROUND - Soft gradients
   ═══════════════════════════════════════════════════════════════════════════ */

.stApp {{
    background: 
        radial-gradient(ellipse at 0% 0%, rgba(102,126,234,0.08) 0%, transparent 50%),
        radial-gradient(ellipse at 100% 0%, rgba(236,72,153,0.05) 0%, transparent 50%),
        radial-gradient(ellipse at 50% 100%, rgba(8,145,178,0.05) 0%, transparent 50%),
...
def generate_planet_qr(planet_name):
    ...
    try:
        ...
        img = qr.make_image(fill_color="#00d4ff", back_color="#0a1628")
...
with st.sidebar:
    ...
    <div style='text-align: center; padding: 10px 0 20px 0;'>
        ...
        <div style='font-size: 1.5rem; font-weight: 700; background: linear-gradient(135deg, #00d4ff, #00ff88); 
                    -webkit-background-clip: text; -webkit-text-fill-color: transparent;'>ATLAS</div>
        <div style='font-size: 0.7rem; opacity: 0.5;'>v21 • Exoplanet Research</div>
    </div>
    """, unsafe_allow_html=True)
    ...
    if theme_cols[0].button("🌙", key="theme_dark_btn", use_container_width=True,
                            type="primary" if st.session_state.theme == 'dark' else "secondary"):
        st.session_state.theme = 'dark'
        st.rerun()
    if theme_cols[1].button("☀️", key="theme_light_btn", use_container_width=True,
    ...
    with stat_cols[0]:
        ...
        <div style='text-align: center;'>
            <div style='font-size: 1.5rem; font-weight: 700;'>{len(st.session_state.saved_systems)}</div>
            <div style='font-size: 0.65rem; opacity: 0.6; text-transform: uppercase;'>Систем</div>
        </div>
        """, unsafe_allow_html=True)
    with stat_cols[1]:
        st.markdown(f"""
        <div style='text-align: center;'>
            <div style='font-size: 1.5rem; font-weight: 700; color: #00ff88;'>{st.session_state.habitable_count}</div>
            <div style='font-size: 0.65rem; opacity: 0.6; text-transform: uppercase;'>Жизнь</div>
        </div>
        """, unsafe_allow_html=True)
    with stat_cols[2]:
        st.markdown(f"""
        <div style='text-align: center;'>
            <div style='font-size: 1.5rem; font-weight: 700;'>{st.session_state.get('total_planets_found', 0)}</div>
            <div style='font-size: 0.65rem; opacity: 0.6; text-transform: uppercase;'>Планет</div>
        </div>
        """, unsafe_allow_html=True)
    ...
    ach_title = {'ru': '🏆 Достижения', 'en': '🏆 Achievements', 'kz': '🏆 Жетістіктер'}
    lang = st.session_state.get('lang', 'ru')
    unlocked = len(st.session_state.achievements_unlocked)
    total = len(ACHIEVEMENTS)
    
    st.markdown(f"### {ach_title.get(lang, ach_title['en'])}")
    st.progress(unlocked / total if total > 0 else 0)
    st.caption(f"{unlocked}/{total}")
    
    # Show last 3 achievements
    if st.session_state.achievements_unlocked:
        for ach_id in st.session_state.achievements_unlocked[-3:]:
            ach = ACHIEVEMENTS.get(ach_id, {})
            st.markdown(f"{ach.get('icon', '🏆')} {ach.get(lang, ach.get('en', ''))}")
    
    st.markdown("---")
    
    # Quick actions
    quick_title = {'ru': '⚡ Быстрый доступ', 'en': '⚡ Quick Access', 'kz': '⚡ Жылдам қол жеткізу'}
    st.markdown(f"### {quick_title.get(lang, quick_title['en'])}")
    
    quick_stars = ["TRAPPIST-1", "TOI-700", "Kepler-442", "Proxima"]
    for star_name in quick_stars:
        scanned = is_star_scanned(star_name)
        if st.button(f"{'✓ ' if scanned else '→ '}{star_name}", key=f"q_{star_name}", 
                     use_container_width=True, disabled=scanned):
    ...
    # Data Management
    data_title = {'ru': '💾 Данные', 'en': '💾 Data', 'kz': '💾 Деректер'}
    st.markdown(f"### {data_title.get(lang, data_title['en'])}")
    
    # Storage status
    if LOCAL_STORAGE_AVAILABLE:
        st.markdown("💚 <span style='color:#00ff88; font-size:0.8rem;'>LocalStorage ✓</span>", unsafe_allow_html=True)
    else:
        st.markdown("🔴 <span style='color:#ff6666; font-size:0.8rem;'>LocalStorage ✗</span>", unsafe_allow_html=True)
        st.caption("pip install streamlit-js-eval")
    
    # Save button
    save_btn_text = {'ru': '💾 Сохранить', 'en': '💾 Save', 'kz': '💾 Сақтау'}
    if st.button(save_btn_text.get(lang, save_btn_text['en']), use_container_width=True):
        save_to_local_storage()
        st.toast("✅ Данные сохранены!" if lang == 'ru' else "✅ Data saved!", icon="💾")
    
    # Clear button with confirmation
    clear_btn_text = {'ru': '🗑️ Очистить всё', 'en': '🗑️ Clear All', 'kz': '🗑️ Барлығын тазалау'}
    with st.expander(clear_btn_text.get(lang, clear_btn_text['en'])):
        confirm_text = {'ru': 'Это удалит все ваши данные!', 'en': 'This will delete all your data!', 'kz': 'Бұл барлық деректеріңізді жояды!'}
        st.warning(confirm_text.get(lang, confirm_text['en']))
        if st.button("⚠️ Confirm / Подтвердить", key="confirm_clear"):
            # Clear session state
            for key in list(st.session_state.keys()):
                if key not in ['theme', 'lang']:
                    del st.session_state[key]
            # Clear localStorage
            clear_local_storage()
            st.rerun()

# Auto-save on each page load
save_to_local_storage()
...
with tabs[0]:
    ...
    with col3:
        ...
        news_header = {'ru': '📰 Последние новости', 'en': '📰 Latest News', 'kz': '📰 Соңғы жаңалықтар'}
...
*— ATLAS v21*
        ...
        for i, forecast in enumerate(forecasts):
            with weather_cols[i]:
                temp_color = '#ff6666' if forecast['temp'] > 100 else '#00d4ff' if forecast['temp'] > 0 else '#66b3ff' if forecast['temp'] > -50 else '#a0c4ff'
                st.markdown(f"""
        ...
        if st.session_state.get('show_qr') == sel_p['name']:
            ...
            <div style='background: linear-gradient(135deg, rgba(0,212,255,0.1), rgba(0,255,136,0.05));
                ...
                <h3 style='color: #00d4ff; margin-bottom: 20px;'>{qr_title}</h3>
            """, unsafe_allow_html=True)
