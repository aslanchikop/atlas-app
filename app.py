...
if os.path.exists(CACHE_FILE):
    try:
        ...
        print(f'[Gemini Cache] Loaded {len(_gemini_cache)} entries from {CACHE_FILE}', flush=True)
    except Exception as e:
        print(f'[Gemini Cache] Failed to load: {e}', flush=True)

def save_gemini_cache():
    try:
        with open(CACHE_FILE, 'w', encoding='utf-8') as f:
            json.dump(_gemini_cache, f, ensure_ascii=False, indent=2)
    except Exception as e:
